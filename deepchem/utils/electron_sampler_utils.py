...
class ElectronSampler:
  ...
  Example
  -------
  >>> from deepchem.utils.electron_sampler_utils import ElectronSampler
  >>> def test_f(x):
    ...
    if self.simultaneous:
      if self.symmetric:
        ...
        for i in range(self.steps):
          ...
          lp2 = self.f(x2)  # log probability of x2 state
          lq1 = self.log_prob_gaussian(self.x, x2, std)  # forward probability
          lq2 = self.log_prob_gaussian(
              x2, self.x, stddev * asymmetric_func(x2))  # backward probability
